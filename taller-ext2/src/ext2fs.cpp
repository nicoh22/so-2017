...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	Ext2FSBlockGroupDescriptor *myBlockgroup = block_group(blockgroup_index);
	unsigned int inodes[block_size];
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	unsigned int cant_direct_blocks = 12;
	unsigned int indirection_block_number;
	if(block_number < 12)
	{
		return inode->blocks[block_number];
	}
	else if(block_number < 12 + cant_entries)
	{
		indirection_block_number = block_number - 12;
	   	read_block(inode->blocks[12], &indirection_blocks);
		return indirection_blocks[indirection_block_number];
	}
	else//ponele
	{
		indirection_block_number = block_number - 12 - cant_entries;
		read_block(inode->blocks[13], &indirection_blocks);
		unsigned int index_block = indirection_block_number % cant_entries;
		unsigned int indirect_block_number = indirection_block_number / cant_entries;
